...
# The budget check deliberately reuses the benchmark harness's helpers
# (mesh build, dry-bed initial state, hydrograph phases) so the measured
# configuration is bit-for-bit what the suite gates on.
# pyright: reportPrivateUsage=false
from inundation.bench import flood_propagation as fp
...
def _measure(nx: int, ny: int, t_end_s: float, output_dir: Path) -> BudgetRow:
    ...
    config = WorkflowConfig(
        ...
        progress=False,
        initial_state_source=fp.initial_state(spec),
